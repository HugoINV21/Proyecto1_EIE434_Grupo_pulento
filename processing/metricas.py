...
def calcular_IAE(errores, dt):    #sumatoria del error absoluto multiplicado por dt
    return np.sum(np.abs(errores)) * dt

def calcular_ISE(errores, dt): #   #sumatoria del error al cuadrado
    return np.sum(np.square(errores)) * dt

def calcular_ITAE(errores, dt):
    t = np.arange(len(errores)) * dt   # arreglo de tiempo
...
def calcular_todas_las_metricas(errores, dt):  # calculamos cada metrica por separado y redondeamos
...
def calcular_mejora(valor_ppo, valor_mask):
    # formula para reduccion porcentual: (Vbase - Vnueva) / Vbase * 100
